...

{
  ...
  noPrecip <- weatherData$relHum[weatherData$precipitation==0]
  Precip <- weatherData$relHum[weatherData$precipitation==1]
  

boxplot_calc <- function(x){
  lower <- quantile(x, probs = 0.25)
  middle <- median(x)
  upper <- quantile(x, probs = 0.75)
  iqr <- upper - lower
  ymin <- lower - (1.5*iqr)
  ymax <- upper + (1.5*iqr)
  mean <- mean(x)
  std <- sd(x)
  return(c(lower,middle, upper,iqr, ymin, ymax, mean, std))
}
  
noPrecip_plot <- boxplot_calc(noPrecip) 
Precip_plot <- boxplot_calc(Precip) 
  ...
  plot2 = ggplot() +     # no dataframe because the data is being manually entered
    ...
    geom_boxplot(stat="identity",     # manually enter data
                 ...
                 mapping=aes(x = 0.75,            # x-position of box
                             ymin = noPrecip_plot[5],        # actual low whisker end
                             lower = noPrecip_plot[1],       # low end of box (1st quartile/0.25 quantile)
                             middle = noPrecip_plot[2],      # median of data
                             upper = noPrecip_plot[3],       # high end of box (3rd quartile/0.75 quantile)
                             ymax =  noPrecip_plot[6]   )) +  # actual high whisker end
    
    # Can use annotate(), geom="segment" to create a line 
    annotate(geom="segment",
             x=0.3,     y=noPrecip_plot[7],      # line starts at (x,y)
             xend=1.2,  yend=noPrecip_plot[7],   # line ends at (xend,yend)
             color = rgb(red=1, green=0.5, blue=0)) +              # all red, half green makes orange
    
    # Can use annotate(), geom="text" to add text 
    annotate(geom="text",
             x = 0.75, y = 59,         # text is at (x,y)
             label = paste0("stdev = ",round(noPrecip_plot[8], digits=3)),  # https://en.wikipedia.org/wiki/List_of_Unicode_characters
             color = "blue") +
  
  # second boxplot
  geom_boxplot(stat="identity",     # manually enter data
               # scroll far down on geom_boxplot() Help page to find these aesthetics
               mapping=aes(x = 1.75,            # x-position of box
                           ymin = Precip_plot[5],        # actual low whisker end
                           lower = Precip_plot[1],       # low end of box (1st quartile/0.25 quantile)
                           middle = Precip_plot[2],      # median of data
                           upper = Precip_plot[3],       # high end of box (3rd quartile/0.75 quantile)
                           ymax =  Precip_plot[6]   )) +  # actual high whisker end
  
  # Can use annotate(), geom="segment" to create a line 
  annotate(geom="segment",
           x=1.3,     y=Precip_plot[7],      # line starts at (x,y)
           xend=2.2,  yend=Precip_plot[7],   # line ends at (xend,yend)
           color = rgb(red=1, green=0.5, blue=0)) +              # all red, half green makes orange
  
  # Can use annotate(), geom="text" to add text 
  annotate(geom="text",
           x = 1.75, y = 72,         # text is at (x,y)
           label = paste0("stdev = ",round(Precip_plot[8], digits=3)),  # https://en.wikipedia.org/wiki/List_of_Unicode_characters
           color = "blue") +
  

    # Need to use scale_x_continuous instead of scale_x_discrete when doing manual boxplots
    scale_x_continuous(breaks = c(0.75, 1.75),
                       labels = c("No Precip", "Precip")) +
    
    theme_classic() + 
    
    labs(title = "Relative Humidity vs Precip",
