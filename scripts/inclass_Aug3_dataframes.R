{
  ...
  month <- unique(weatherData$month)
  heatMonth <- c(rep(0,12))
  coolMonth <- c(rep(0,12))
  ...
  for (i in 1:nrow(weatherData))
    {
      ...
      {
        # check if the month
        if(weatherData$month[i] == month[j])
        {
          heatMonth[j] = heatMonth[j] + weatherData$heatDays[i]; 
          coolMonth[j] = heatMonth[j] + weatherData$coolDays[i]; 
          break;
        }
      }
    } 
  ...
  niceDf <-  cbind(month,heatMonth, coolMonth)
