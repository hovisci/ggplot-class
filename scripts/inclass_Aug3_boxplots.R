{
  ...
  # Create empty vectors to be populated by for loop
  q25 = q75 = whiskerHigh = whiskerLow = yMed = yMean = ySD = c();
  
  

  for (i in 1:2)
  { 
    x <-  weatherData$relHum[weatherData$precip == (i-1)]; # data[i], stole i-1 from answer
                             
    q25[i] <- quantile(x, probs = 0.25); # calc 1st quantile
    q75[i] <- quantile(x, probs = 0.75); # calc 3rd quantile
    
    IQR <- q75[i] - q25[i]; # calc IQR 
    
    whiskerHighT <- q75[i] + 1.5*IQR;
    whiskerLowT <- q25[i] - 1.5*IQR;
    
    xHigh = x[x > q75[i] & x <= whiskerHighT]; # y such that y is between 3/4 and high whisker point
    whiskerHigh[i] = max(xHigh);               # maximum of these values (becomes the high whisker point)
    xLow = x[x >= whiskerLowT & x < q25[i]];   # y such that y is between low whisker point and 1/4
    whiskerLow[i] = min(xLow);                 # minimum of these values (becomes the low whisker point)
  ...
  # NOTE:  for variable[i], i = 1 is no precip, i=2 is precip 
